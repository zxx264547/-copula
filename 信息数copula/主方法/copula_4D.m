...
u1 = ksdensity(x1,x1,'function','cdf','Bandwidth',1);
u2 = ksdensity(x2,x2,'function','cdf','Bandwidth',1);
u3 = ksdensity(x3,x3,'function','cdf','Bandwidth',1);
u4 = ksdensity(x4,x4,'function','cdf','Bandwidth',1);
% 控制u范围在0,1之间
u1(u1>=1) = 0.999;
u2(u2>=1) = 0.999;
u3(u3>=1) = 0.999;
...
% 插值 z 值，以适应网格
Z = griddata(x2,x3, x1_up, X, Y, 'cubic');
s=0.8;
surf(X, Y, Z,'FaceAlpha',s, 'FaceColor', 'red');
hold on
Z = griddata(x2, x3, x1_low, X, Y, 'cubic');
surf(X, Y, Z,'FaceAlpha',s, 'FaceColor', 'red');
shading interp
...
accuracy=length(same_list)/length(intersect(real_out_list,Data(:,5)));
% 误识别率
same_list = intersect(real_norm_list,recog_out_list );
error=length(same_list)/length(intersect(real_norm_list,Data(:,5)));

%out1的识别率
list_out1_recog_correct  = intersect(out1_list,recog_out_list );%算法识别出来的out1的数据索引
acc_1=length(list_out1_recog_correct )/length(intersect(out1_list,Data(:,5)));

%out2的识别率
list_out2_recog_correct  = intersect(out2_list,recog_out_list );%算法识别出来的out1的数据索引
acc_2=length(list_out2_recog_correct )/length(intersect(out2_list,Data(:,5)));

%out3的识别率
list_out3_recog_correct  = intersect(out3_list,recog_out_list );%算法识别出来的out1的数据索引
acc_3=length(list_out3_recog_correct )/length(intersect(out3_list,Data(:,5)));

%out4的识别率
list_out4_recog_correct  = intersect(out4_list,recog_out_list );%算法识别出来的out1的数据索引
acc_4=length(list_out4_recog_correct )/length(intersect(out4_list,Data(:,5)));
